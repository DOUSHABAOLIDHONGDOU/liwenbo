...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    pos = 0
    for i in range(1, 11):
        ma = make_averaged(roll_dice, trials_count)(i, dice)
        if ma > maxx:
            maxx = ma
            pos = i
    return pos
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
#     如果掷骰子（或使用 Free Bacon）能够触发额外回合，玩家将选择不掷骰子（即掷 0 次骰子）。
# 如果掷骰子（或使用 Free Bacon）不会触发额外回合，但可以获得至少 cutoff 的分数，玩家将选择不掷骰子（即掷 0 次骰子）。
# 如果既没有触发额外回合，也没有达到 cutoff，则选择掷 num_rolls 次骰子
    # BEGIN PROBLEM 11
...
def final_strategy(score, opponent_score):
    ...
    *** 如果 Free Bacon 得分大于或等于 8 则直接使用 Free Bacon
      如果分数接近 100 选择掷少量骰子_狗一下
      如果对手的分数接近 100 可以选择更多的骰子来提高得分_赌一把
      ***
    """
